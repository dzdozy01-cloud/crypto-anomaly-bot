...
**Scoring latency p95 ≈ 65 ms**, well inside the 200 ms budget. 302 tests passing.

> ⚠️ **On the accuracy numbers.** You will see precision 0.99 / recall 0.93 from
> `cadb evaluate`. That is measured against synthetic scenarios generated by
> *this same codebase*, so it is **not evidence of a trading edge** — see
> [Detection quality](#detection-quality) for a demonstration that four lines of
> numpy score nearly as well. Treat it as a regression harness, not validation.
...
**Queries work for every listed token.** The streaming engine only subscribes to
what discovery flags, but `/check`, `/explain` and `/book` fall back to REST when
a symbol is not currently streamed — so any of the ~10,000 listed pairs can be
analysed on demand, and a quiet BTC returns a real report rather than "no data".
`/movers` ranks the biggest moves across all venues in one call.

**No pinned symbols by default.** `exchange.symbols` ships empty. Pinning majors
meant watching BTC/ETH/SOL whether or not anything was happening there — books far
too deep to move on one actor's flow — which produced constant low-value alerts
while ignoring the low-cap pairs where manipulation actually occurs. Discovery
scans every listed pair on every venue and subscribes only what is genuinely
anomalous (|24h move| ≥ 25%, volume surge, or a new listing).
...
liquidity band that filters out both illiquid noise and unmovable majors. Discovery scans
**USDT, USDC and USD** quotes — Coinbase and Kraken price in USD/USDC, so a
USDT-only scan saw just 3% and 6% of their markets. A live sweep now covers
**8,884 pairs across seven reachable venues**, de-duplicated to one slot per base
asset so BTC/USDT and BTC/USDC never consume two streams a static list
...
**Bad-data rejection.** Exchange ticker fields cannot be taken at face value.
Two independent guards run before anything is flagged: a venue's reported 24h
change must agree with the value implied by its own open/last prices, and any
move beyond ±50%% must be corroborated by at least one other venue listing the
same asset. A single Binance sweep once reported ~20 tokens at −55%% to −75%%
while the identical assets sat at +1%% to +4%% on MEXC; unfiltered, that
artefact crowded every genuine mover out of the watchlist.
...
**This is circular, and the size of the circularity is measurable.** The
`generate_labelled_set` function that produces the "ground truth" and the
`RuleEngine` that scores it were written by the same author, against the same
feature names, with the same thresholds in mind. The generator emits
`volume_z ∈ [3.5, 12]` for a pump *because* the rule fires above 3.0.

The clearest way to see this: run trivial detectors against the same benchmark.

| Detector | Precision | Recall | F1 |
|---|---|---|---|
| `volume_z > 3` (one line) | 0.886 | 0.740 | 0.806 |
| `\|obi\| > 0.35` (one line) | 0.983 | 0.502 | 0.665 |
| Both, OR'd (two lines) | 0.900 | 0.867 | 0.883 |
| Four thresholds OR'd | 0.888 | **0.958** | **0.921** |
| **Full 20-feature system + IsolationForest** | 0.987 | 0.922 | **0.953** |

Four lines of numpy reach F1 0.921 against the full pipeline's 0.953. A benchmark
where a trivial baseline nearly matches the system is measuring **the generator,
not the detector**.

### What the harness is legitimately good for

* **Regression testing.** It reliably catches breakage during refactoring — it
  caught the log-normal z-score bug, the organic-discount suppressing flash
  crashes, and the coverage damping that silenced single-source events. Every
  one of those was a real defect found by this harness.
* **Cold-start.** The IsolationForest needs *a* distribution before it can call
  anything anomalous.
* **Encoding domain assumptions explicitly**, where they can be argued with.

### What it is not

It is **not** external validation, and no claim of real-world accuracy should be
read into it. Specifically untested: whether these features separate
manipulation from legitimate volatility on real order books; whether the
thresholds hold across market regimes; what the false-positive rate is on a
normal trading day; whether detection precedes the price move usefully.

### How to actually validate this

The honest path is labelled real incidents:

1. **Record live telemetry** — `backtest.EventRecorder` writes JSONL from the bus.
2. **Label known events** — CFTC/SEC manipulation cases, exchange delisting
   notices, documented rug-pulls (Chainalysis and Solidus Labs publish datasets),
   or post-hoc obvious pumps on low-cap pairs.
3. **Replay and measure** — `cadb backtest events.jsonl` against those labels.
4. **Report that number instead of this one.**

Until step 4 exists, the defensible claim is: *"the pipeline detects the patterns
it was designed to detect, at sub-200 ms latency, without crashing."* That is a
working system, not a proven edge.

## Telegram bot

25 commands across four groups. The bot is not just an alert pipe — you can
...
| `/check <ASSET>` | Full report for **any listed token** — streamed or fetched on demand |
| `/movers [n]` | Biggest movers right now across every venue |
...
| `/watchlist` | Discovery status — pairs scanned per venue, scan age, stream count |
...
```bash
pytest tests/ -v                                  # 302 tests, ~2 min
