...
def _cmd_evaluate(args: argparse.Namespace) -> int:
    ...
    print(
        "\n⚠️  SYNTHETIC BENCHMARK — these scenarios were generated by this same\n"
        "    codebase, so the scores measure self-consistency, not real-world\n"
        "    accuracy. Use for regression testing only. See README > Detection\n"
        "    quality for trivial-baseline comparisons and how to validate\n"
        "    against real labelled incidents.\n"
    )
    ...
    # Trivial baselines: if a two-line threshold rule matches the full pipeline,
    # the benchmark is measuring the generator rather than the detector.
    from .modules.ml.features import FEATURE_NAMES

    fi = {n: i for i, n in enumerate(FEATURE_NAMES)}

    def _pr(pred: np.ndarray) -> tuple[float, float, float]:
        tp = int((pred & (y == 1)).sum())
        fp = int((pred & (y == 0)).sum())
        fn = int((~pred & (y == 1)).sum())
    ...
    print(f"\n{'baseline (sanity check)':<40} {'prec':>7} {'recall':>7} {'f1':>7}")
    print("-" * 64)
    for label, pred in [
        ("volume_z > 3  (one line)", X[:, fi["volume_z"]] > 3.0),
        ("|obi| > 0.35  (one line)", np.abs(X[:, fi["obi"]]) > 0.35),
        (
            "four thresholds OR'd",
            (X[:, fi["volume_z"]] > 2.5)
            | (np.abs(X[:, fi["obi"]]) > 0.3)
            | (X[:, fi["bot_farm_score"]] > 0.5)
            | (X[:, fi["liquidity_drop"]] > 0.3),
        ),
        ("FULL SYSTEM (threshold 80)", scores >= 80),
    ]:
        p, r, f1 = _pr(pred)
        print(f"{label:<40} {p:>7.3f} {r:>7.3f} {f1:>7.3f}")
    print(
        "\nIf the trivial baselines approach the full system, the benchmark is\n"
        "measuring the data generator, not detection skill."
    )
